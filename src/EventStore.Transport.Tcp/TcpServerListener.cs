...
namespace EventStore.Transport.Tcp
{
    ...
    {
        private static readonly ILogger Log = TraceLogger.GetLogger<TcpServerListener>();
        ...
        public void StartListening(Action<IPEndPoint, Socket> callback, string securityType)
        {
            ...
            if (infoEnabled) Log.LogInformation("Starting {0} TCP listening on TCP endpoint: {1}.", securityType, _serverEndPoint);
            try
            {
            ...
            {
                if (infoEnabled) Log.LogInformation("Failed to listen on TCP endpoint: {0}.", _serverEndPoint);
