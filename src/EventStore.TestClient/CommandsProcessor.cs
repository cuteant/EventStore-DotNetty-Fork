...
namespace EventStore.TestClient
{
  public class CommandsProcessor
  {
    ...
    public bool TryProcess(CommandProcessorContext context, string[] args, out int exitCode)
    {
      ...
      {
        _log.LogInformation("Unknown command: {0}.", commandName);
      ...
      ThreadPool.QueueUserWorkItem(_ =>
      {
        try
        {
          ...
          {
            exitC = 1;
            _log.LogInformation("Usage of {0}:{1}{2}", commandName, Environment.NewLine, commandProcessor.Usage);
          }
          executedEvent.Set();
        }
        catch (Exception exc)
        {
          _log.LogError(exc, "Error while executing command {0}.", commandName);
          exitC = -1;
          executedEvent.Set();
        }
      });
      ...
      if (!string.IsNullOrWhiteSpace(context.Reason))
        _log.LogError("Error during execution of command: {0}.", context.Reason);
      if (context.Error != null)
      {
        _log.LogError(context.Error, "Error during execution of command");

        var details = new StringBuilder();
        BuildFullException(context.Error, details);
        _log.LogError("Details: {0}", details.ToString());
      }
