    ...
    internal class DeleteProcessor : ICmdProcessor
    {
        ...
        public bool Execute(CommandProcessorContext context, string[] args)
        {
            ...
            context.Client.CreateTcpConnection(
                ...
                {
                    context.Log.LogInformation("[{0}, L{1}]: Trying to delete event stream '{2}'...", conn.RemoteEndPoint, conn.LocalEndPoint, eventStreamId);
                ...
                {
                    sw.Stop();
                    context.Log.LogInformation("Delete request took: {0}.", sw.Elapsed);
                    ...
                    {
                        context.Log.LogInformation("DELETED event stream {0}.", eventStreamId);
                        PerfUtils.LogTeamCityGraphData(string.Format("{0}-latency-ms", Keyword), (int)Math.Round(sw.Elapsed.TotalMilliseconds));
                        context.Success();
                    }
                    else
                    {
                        context.Log.LogInformation("DELETION FAILED for event stream {0}: {1} ({2}).", eventStreamId, dto.Message, dto.Result);
                        context.Fail();
                    }
                    conn.Close();
                },
