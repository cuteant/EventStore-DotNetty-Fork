  ...
  internal abstract class ScenarioBase : IScenario
  {
    ...
    public void Run()
    {
      ...
      for (int i = 0; i < Connections; ++i)
      {
        ...
        _connections[i].Closed += (s, e) => Log.LogDebug("[SCENARIO] {0} closed.", e.Connection.ConnectionName);
        _connections[i].Connected += (s, e) => Log.LogDebug("[SCENARIO] {0} connected to [{1}].", e.Connection.ConnectionName, e.RemoteEndPoint);
        _connections[i].Disconnected += (s, e) => Log.LogDebug("[SCENARIO] {0} disconnected from [{1}].", e.Connection.ConnectionName, e.RemoteEndPoint);
        _connections[i].Reconnecting += (s, e) => Log.LogDebug("[SCENARIO] {0} reconnecting.", e.Connection.ConnectionName);
        _connections[i].ErrorOccurred += (s, e) => Log.LogDebug(e.Exception, "[SCENARIO] {0} error occurred.", e.Connection.ConnectionName);
    ...
    private void DeleteDatabase()
    {
      try
      {
        if (_dbPath != null)
        {
          Log.LogInformation("Deleting {0}...", _dbPath);
          Directory.Delete(_dbPath, true);
          Log.LogInformation("Deleted {0}", _dbPath);
        }
      }
    ...
    {
      Log.LogInformation("Writing. Mode : {0,-15} Streams : {1,-10} Events per stream : {2,-10}",
               mode,
               streams.Length,
               eventsPerStream);
      ...
      {
        Task.WaitAll(tsks);
        Log.LogInformation("Finished writing. Mode : {0,-15} Streams : {1,-10} Events per stream : {2,-10}",
                       mode,
                       streams.Length,
                       eventsPerStream);
      });
    ...
    {
      Log.LogInformation("Deleting streams...");
      ...
      {
        var s = stream;
        Log.LogInformation("Deleting stream {0}...", stream);
        var task = store.DeleteStreamAsync(stream, (EventsPerStream - 1), hardDelete: true)
                        .ContinueWith(x => Log.LogInformation("Stream {0} successfully deleted", s));

        tasks.Add(task);
      }
      Task.WaitAll(tasks.ToArray());

      Log.LogInformation("All streams successfully deleted");
    }

    protected Task CheckStreamsDeleted(IEnumerable<string> streams)
    {
      Log.LogInformation("Verifying streams are deleted...");
      ...
      {
        Task.WaitAll(tsks);
        Log.LogInformation("Stream deletion verification succeeded.");
      });
    }
    ...
    protected Task Read(string[] streams, int @from, int count)
    {
      ...
      {
        Task.WaitAll(tsks);
        Log.LogInformation("Done reading [{0}]", string.Join(",", streams));
      });
    }
    ...
    private int StartNewNode()
    {
      ...
      {
        Log.LogInformation("Mono at {0} will be used.", pathToMono);
        fileName = pathToMono;
      ...
      Log.LogInformation("Starting [{0} {1}]...", fileName, arguments);
      ...
      Log.LogInformation("Started node with process id {0}", nodeProcess.Id);

      Thread.Sleep(StartupWaitInterval);
      Log.LogInformation("Started [{0} {1}]", fileName, arguments);
    ...
    protected void KillNode(int processId)
    {
      if (processId != -1)
        ...
        Log.LogInformation("Skip killing, procId -1");
    }

    private void KillStartedNode(int processId)
    {
      Log.LogInformation("Killing {0}...", processId);
      ...
      if (TryGetProcessById(processId, out process))
      {
        ...
        if (process.HasExited)
        {
          ...
          Log.LogInformation("Killed process {0}, wait a bit.", processId);
        ...
        else
        {
          ...
          if (TryGetProcessById(processId, out temp))
            ...
            Log.LogInformation("Process {0} did not report about exit in time but is not found again.", processId);
        }
      }
      else
        Log.LogError("Process {0} was not found to be killed.", processId);
    }
    ...
    {
      Log.LogInformation("Killing remaining nodes...");
      try
      {
      ...
      {
        Log.LogError("Failed to kill started nodes: {0}.", ex.Message);
      }
    }

    protected void Scavenge()
    {
      Log.LogInformation("Send scavenge command...");
      ...
      Log.LogInformation("Scavenge command was sent.");
    }
    ...
    private Task WriteSingleEventAtTime(string stream, int events, Func<int, EventData> createEvent)
    {
      ...
      {
        Log.LogInformation("WriteSingleEventAtTime for stream {0} failed.", stream);
      ...
      writeSingleEvent = _ =>
      {
        ...
        {
          Log.LogInformation("Wrote {0} events to [{1}]", events, stream);
    ...
    private Task WriteBucketOfEventsAtTime(string stream, int eventCount, Func<int, EventData> createEvent)
    {
      ...
      Action<Task> fail = prevTask =>
      {
        Log.LogInformation("WriteBucketOfEventsAtTime for stream {0} failed.", stream);
        resSource.SetException(prevTask.Exception);
      };
      ...
      writeBatch = _ =>
      {
        ...
        {
          Log.LogInformation("Wrote {0} events to [{1}] ({2} events at once)", eventCount, stream, bucketSize);
          resSource.SetResult(null);
          return;
        }
    ...
    private Task WriteEventsInTransactionalWay(string stream, int eventCount, Func<int, EventData> createEvent)
    {
      ...
      Action<Task> fail = prevTask =>
      {
        Log.LogInformation("WriteEventsInTransactionalWay for stream {0} failed.", stream);
        resSource.SetException(prevTask.Exception);
      };
      ...
      writeTransactionEvent = prevTask =>
      {
        if (writtenCount == eventCount)
        {
          ...
          commitTask.ContinueWith(t =>
                {
                Log.LogInformation("Wrote {0} events to [{1}] (in single transaction)", eventCount, stream);
                resSource.SetResult(null);
    ...
    private Task ReadStream(string stream, int from, int count)
    {
      ...
      Action<Task> fail = prevTask =>
      {
        Log.LogInformation("ReadStream for stream {0} failed.", stream);
        resSource.SetException(prevTask.Exception);
      };
      ...
      readTask.ContinueWith(t =>
      {
        try
        {
          ...
          Log.LogInformation("Done reading [{0}] from {1,-10} count {2,-10}", stream, from, count);
          resSource.SetResult(null);
        }
        catch (Exception exc)
        {
