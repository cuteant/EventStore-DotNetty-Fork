...
namespace EventStore.TestClient.Commands.RunTestScenarios
{
  internal abstract class ProjectionsScenarioBase : ScenarioBase
  {
    ...
    private Dictionary<string, string> GetProjectionStatistics(string projectionName)
    {
      ...
      {
        Log.LogInformation(ex, "Failed to read projection statistics. Will continue.");
        rawState = null;
      }

      Log.LogInformation("Raw {0} stats: {1}", projectionName, rawState);
    ...
    private Dictionary<string, string> GetProjectionState(string projectionName)
    {
      ...
      Log.LogInformation("Raw {0} state: {1}", projectionName, rawState);
    ...
    private string GetProjectionStateSafe(string projectionName)
    {
      ...
      {
        rawState = null;
        Log.LogInformation(ex, "Failed to get projection state");
      }
      return rawState;
    }
    ...
    protected void EnableProjectionByCategory()
    {
      ...
      while (retryCount < 5)
      {
        ...
        try
        {
          ...
          if (!isRunning)
          {
            Log.LogDebug(string.Format("Enable *{0}* projection", byCategoryProjection));
            GetProjectionsManager().EnableAsync(byCategoryProjection, AdminCredentials).Wait();
          }
          else
          {
            Log.LogDebug(string.Format("Already enabled *{0}* projection", byCategoryProjection));
          }
