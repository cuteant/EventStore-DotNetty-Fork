...
namespace EventStore.ClientAPI
{
    ...
    public class StreamMetadata
    {
                                  ...
                                  StreamAcl acl, IDictionary<string, JToken> customMetadata = null)
        {
            if (maxCount <= 0)
            {
                CoreThrowHelper.ThrowArgumentOutOfRangeException_StreamMetadata_MaxCount();
            }
            if (maxAge <= TimeSpan.Zero)
            {
                CoreThrowHelper.ThrowArgumentOutOfRangeException_StreamMetadata_MaxAge();
            }
            if (truncateBefore < 0)
            {
                CoreThrowHelper.ThrowArgumentOutOfRangeException_StreamMetadata_TruncateBefore();
            }
            if (cacheControl <= TimeSpan.Zero)
            {
                CoreThrowHelper.ThrowArgumentOutOfRangeException_StreamMetadata_CacheControl();
            }
        ...
        public T GetValue<T>(string key)
        {
            ...
            {
                CoreThrowHelper.ThrowArgumentException_NoKeyFoundInCustomMetadata(key);
            }
            return res;
        }
        ...
        {
            if (!TryGetValueAsRawJsonString(key, out string res))
            {
                CoreThrowHelper.ThrowArgumentException_NoKeyFoundInCustomMetadata(key);
            }
            return res;
        ...
        {
            if (reader.TokenType != type) { CoreThrowHelper.ThrowException_InvalidJson(); }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void Check(bool read, JsonTextReader reader)
        {
            if (!read) { CoreThrowHelper.ThrowException_InvalidJson(); }
        }
    }
}
