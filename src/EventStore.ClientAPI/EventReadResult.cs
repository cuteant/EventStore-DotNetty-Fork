...
namespace EventStore.ClientAPI
{
  ...
  public class EventReadResult
  {
    ...
    private EventReadResult(EventReadStatus status, string stream, long eventNumber, ResolvedEvent? resolvedEvent)
    {
      Ensure.NotNullOrEmpty(stream, nameof(stream));
    ...
    internal static EventReadResult Create(EventReadStatus status, string stream, long eventNumber, ResolvedEvent? resolvedEvent)
    {
      return new EventReadResult(status, stream, eventNumber, resolvedEvent: resolvedEvent);
    }
  }
}