...
namespace EventStore.ClientAPI.SystemData
{
  ...
  internal struct TcpPackage
  {
    ...
    public static TcpPackage FromArraySegment(ArraySegment<byte> data)
    {
      ...
      {
        throw new ArgumentException($"ArraySegment too short, length: {data.Count}", "data");
      }
      ...
      if ((flags & TcpFlags.Authenticated) != 0)
      {
        ...
        login = Helper.UTF8NoBom.GetStringWithBuffer(data.Array, data.Offset + AuthOffset + 1, loginLen);
        ...
        pass = Helper.UTF8NoBom.GetStringWithBuffer(data.Array, data.Offset + AuthOffset + 1 + loginLen + 1, passLen);
    ...
    public byte[] AsByteArray()
    {
      if ((Flags & TcpFlags.Authenticated) != 0)
      {
        ...
        if (loginLen > 255) throw new ArgumentException($"Login serialized length should be less than 256 bytes (but is {loginLen}).");
        if (passLen > 255) throw new ArgumentException($"Password serialized length should be less than 256 bytes (but is {passLen}).");
