...
namespace EventStore.Projections.Core.v8
{
    public class PreludeScript : IDisposable
    {
        ...
        private readonly ILogger Log = TraceLogger.GetLogger<PreludeScript>();
        ...
        public PreludeScript(string script, string fileName, Func<string, Tuple<string, string>> getModuleSourceAndFileName,
        ...
        private void LogHandler(string message)
        {
            ...
            {
                if (Log.IsDebugLevelEnabled()) { Log.LogDebug(message); }
            }
        }
