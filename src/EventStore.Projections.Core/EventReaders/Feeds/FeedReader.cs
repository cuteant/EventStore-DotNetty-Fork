...
namespace EventStore.Projections.Core.EventReaders.Feeds
{
                              ...
                              IHandle<EventReaderSubscriptionMessage.NotAuthorized>
    {
            ...
            int maxEvents, Guid requestCorrelationId, IEnvelope replyEnvelope, ITimeProvider timeProvider)
        {
            ...
            if (null == querySource) { ThrowHelper.ThrowArgumentNullException(ExceptionArgument.querySource); }
            if (null == fromPosition) { ThrowHelper.ThrowArgumentNullException(ExceptionArgument.fromPosition); }
            if (null == replyEnvelope) { ThrowHelper.ThrowArgumentNullException(ExceptionArgument.replyEnvelope); }
            if (maxEvents <= 0) { ThrowHelper.ThrowArgumentOutOfRangeException_Positive(ExceptionArgument.maxEvents); }
