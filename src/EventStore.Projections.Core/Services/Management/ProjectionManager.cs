...
namespace EventStore.Projections.Core.Services.Management
{
        ...
        IHandle<ProjectionManagementMessage.ReaderReady>
    {
        ...
        private static readonly ILogger _logger = TraceLogger.GetLogger<ProjectionManager>();
        ...
        private readonly RequestResponseDispatcher<ClientMessage.ReadStreamEventsForward, ClientMessage.ReadStreamEventsForwardCompleted> _readForwardDispatcher;
        ...
        public void Handle(ProjectionManagementMessage.Command.Enable message)
        {
            ...
            if (projection == null)
            {
                _logger.LogError("DBG: PROJECTION *{0}* NOT FOUND.", message.Name);
                message.Envelope.ReplyWith(new ProjectionManagementMessage.NotFound());
            }
            else
            {
        ...
        public void Handle(ProjectionManagementMessage.Command.GetStatistics message)
        {
            ...
            if (!string.IsNullOrEmpty(message.Name))
            {
                ...
                {
                    message.Envelope.ReplyWith(new ProjectionManagementMessage.Statistics(new[] { projection.GetStatistics() }));
                }
            }
            else
            {
        ...
        {
            if (_projectionsMap.TryGetValue(message.ProjectionId, out string name))
            {
        ...
        {
            if (_awaitingSlaveProjections.TryGetValue(message.ProjectionId, out Action<CoreProjectionManagementMessage.SlaveProjectionReaderAssigned> action))
            {
                action(message);
            }
        }
        ...
        private void StartWhenConditionsAreMet()
        {
            ...
            else
            {
                if (_started)
                {
                    if (_logger.IsDebugLevelEnabled()) _logger.LogDebug("PROJECTIONS: Stopping Projections Manager. (Node State : {0})", _currentState);
                    Stop();
                }
            }
        }
        ...
        {
            return _projections.TryGetValue(name, out ManagedProjection result) ? result : null;
        }

        private void StartExistingProjections(Action completed)
        {
            var registeredProjections = new Dictionary<string, long>(StringComparer.Ordinal);
        ...
        {
            if (_logger.IsDebugLevelEnabled()) _logger.LogDebug("PROJECTIONS: Reading Existing Projections from {0}", projectionsRegistrationStreamId);
            var corrId = Guid.NewGuid();
        ...
        private void OnProjectionsListReadCompleted(ClientMessage.ReadStreamEventsForwardCompleted msg, IDictionary<string, long> registeredProjections, long requestedFrom, Action completedAction)
        {
            switch (msg.Result)
            {
                    ...
                    foreach (var evnt in msg.Events)
                    {
                        ...
                        if (evnt.Event.EventType == ProjectionEventTypes.ProjectionCreated)
                        {
                            if (registeredProjections.ContainsKey(projectionName))
                            {
                                ...
                                _logger.LogWarning("PROJECTIONS: The following projection: {0} has a duplicate created event. Using projection Id {1}", projectionName, projectionId);
                                continue;
                            }
                ...
                case ReadStreamResult.AccessDenied:
                    _logger.LogCritical("There was an error reading the projections list due to {0}. Projections could not be loaded.", msg.Result);
                    return;
            }
        ...
        private void StartRegisteredProjections(IDictionary<string, long> registeredProjections, Action completedAction)
        {
            ...
            {
                if (debugEnabled) _logger.LogDebug("PROJECTIONS: No projections were found in {0}, starting from empty stream", ProjectionNamesBuilder.ProjectionsRegistrationStream);
        ...
        private void WriteProjectionsInitializedCompleted(ClientMessage.WriteEventsCompleted completed, Guid registrationEventId, Action action)
        {
            switch (completed.Result)
            {
                ...
                case OperationResult.PrepareTimeout:
                    ...
                    break;
                default:
                    _logger.LogCritical("Cannot initialize projections subsystem. Cannot write a fake projection");
                    break;
            }
        }
        ...
        private ManagedProjection CreateManagedProjectionInstance(string name,
          long projectionId, Guid projectionCorrelationId, Guid workerID, bool isSlave = false,
          Guid slaveMasterWorkerId = default(Guid), Guid slaveMasterCorrelationId = default(Guid))
        {
            ...
            if (_logger.IsDebugLevelEnabled()) _logger.LogDebug("Adding projection {0}@{1} to list", projectionCorrelationId, name);
          ...
          Guid eventId, Action<long> completed, string name, string eventStreamId, IEnvelope replyEnvelope, int retryCount)
        {
            ...
            if (infoEnabled)
            {
                _logger.LogInformation("Projection '{0}' registration has not been written to {1}. Error: {2}",
                    name, eventStreamId, Enum.GetName(typeof(OperationResult), message.Result));
            }
        ...
        private static void CheckSlaveProjectionsStarted(ProjectionManagementMessage.Command.StartSlaveProjections message,
          ref int counter, Dictionary<string, SlaveProjectionCommunicationChannel[]> result)
        {
            counter--;
            if (counter == 0)
            {
        ...
        private void CINP(ProjectionManagementMessage.Command.StartSlaveProjections message,
          SlaveProjectionDefinitions.Definition @group, SlaveProjectionCommunicationChannel[] resultArray,
          int queueIndex, int arrayIndex, Action completed)
        {
