...
namespace EventStore.Projections.Core.Services.Management
{
      ...
      IHandle<ProjectionManagementMessage.ReaderReady>
  {
    ...
    {
      if (_logger.IsDebugLevelEnabled()) _logger.LogDebug("PROJECTIONS: Reading/Starting Existing Projections from $projections-$all");
        ...
        Action completedAction)
    {
      ...
      if (requestedFrom == -1) // first chunk
      {
        if (!anyFound)
        {
          if (_logger.IsDebugLevelEnabled()) _logger.LogDebug("PROJECTIONS: No projections were found in {0}, starting from empty stream", completed.EventStreamId);
        ...
        int retryCount)
    {
          ...
          || message.Result == OperationResult.WrongExpectedVersion)
      {
        if (retryCount > 0)
        {
          if (_logger.IsInformationLevelEnabled()) _logger.LogInformation("Retrying write projection registration for {0}", name);
