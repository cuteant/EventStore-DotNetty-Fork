...
namespace EventStore.Projections.Core.Services.Processing
{
    public class EventByTypeIndexEventReader : EventReader
    {
        ...
        {
            if (_disposed || PauseRequested || Paused) { return; }
        ...
        {
            if (!_fromPositions.TryGetValue(eventStreamId, out long streamPosition))
            {
                streamPosition = -1;
            }
        ...
        private abstract class State : IDisposable
        {
            ...
            protected void DeliverEvent(float progress, ResolvedEvent resolvedEvent, in TFPos position)
            {
                ...
                if (resolvedEvent.IsLinkToDeletedStream && !resolvedEvent.IsLinkToDeletedStreamTombstone) { return; }

                bool isDeletedStreamEvent = StreamDeletedHelper.IsStreamDeletedEventOrLinkToStreamDeletedEvent(
                    resolvedEvent, out string deletedPartitionStreamId);
...

        {
            ...
            public void Handle(ClientMessage.ReadStreamEventsForwardCompleted message)
            {
                ...
                if (!_validRequests.Contains(message.CorrelationId)) { return; }
            ...
            {
                if (!_buffers.TryGetValue(positionEvent.EventStreamId, out Queue<PendingEvent> queue))
                {
            ...
            private void RequestCheckpointStream(bool delay)
            {
                ...
                if (_indexCheckpointStreamRequested) { return; }
            ...
            private void RequestEvents(string stream, bool delay)
            {
                ...
                if (_buffers.TryGetValue(stream, out Queue<PendingEvent> queue) && queue.Count > 0) { return; }
            ...
            private bool ShouldSwitch()
            {
                ...
                var shouldSwitch = _lastKnownIndexCheckpointPosition != null
                           && _streamToEventType.Keys.All(
                               v =>
                               _eofs[v]
                               || _buffers.TryGetValue(v, out Queue<PendingEvent> q) && q.Count > 0
            ...
            IHandle<ProjectionManagementMessage.Internal.ReadTimeout>
        {
            ...
            public void Handle(ClientMessage.ReadAllEventsForwardCompleted message)
            {
                ...
                if (!_tfEventsRequested) { throw new InvalidOperationException("TF events has not been requested"); }
                if (_reader.Paused) { throw new InvalidOperationException("Paused"); }
                _reader._lastPosition = message.TfLastCommitPosition;
            ...
            {
                if (_disposed) { throw new InvalidOperationException("Disposed"); }
                if (_reader.PauseRequested || _reader.Paused) { throw new InvalidOperationException("Paused or pause requested"); }
                if (_tfEventsRequested) { return; }
