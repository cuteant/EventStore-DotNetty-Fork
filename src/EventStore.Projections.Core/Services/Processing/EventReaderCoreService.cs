...
namespace EventStore.Projections.Core.Services.Processing
{
        ...
        IHandle<ReaderCoreServiceMessage.ReaderTick>
    {
        ...
        public void Handle(ReaderSubscriptionManagement.Pause message)
        {
            ...
            if (!_subscriptions.TryGetValue(message.SubscriptionId, out IReaderSubscription projectionSubscription))
            {
        ...
        {
            if (_stopped) { return; }
            if (_runHeadingReader && _headingEventReader.Handle(message)) { return; }
            if (!_eventReaderSubscriptions.TryGetValue(message.CorrelationId, out Guid projectionId))
            {
                return; // unsubscribed
            }
        ...
        private void StopReaders()
        {
            ...
            if (_runHeadingReader) { _headingEventReader.Stop(); }
            _stopped = true;
        }
            ...
            ReaderSubscriptionMessage.CommittedEventDistributed message, Guid projectionId)
        {
            ...
            if (_pausedSubscriptions.Contains(projectionId)) { return false; }

            var projectionSubscription = _subscriptions[projectionId];

            if (!_headingEventReader.TrySubscribe(
