...
namespace EventStore.Projections.Core.Services.Processing
{
        ...
        IEventProcessingProjectionPhase
    {
        ...
        public string TransformCatalogEvent(EventReaderSubscriptionMessage.CommittedEventReceived message)
        {
            switch (_state)
            {
                ...
                case PhaseState.Stopped:
                    _logger.LogError("Ignoring committed catalog event in stopped state");
                    return null;
                default:
            ...
            EventReaderSubscriptionMessage.CommittedEventReceived message, string partition)
        {
            switch (_state)
            {
                case PhaseState.Running:
                    ...
                    return result;
                case PhaseState.Stopped:
                    _logger.LogError("Ignoring committed event in stopped state");
                    return null;
                default:
                    throw new NotSupportedException();
            }
            ...
            string partition, EventReaderSubscriptionMessage.CommittedEventReceived message)
        {
            ...
            var hasBeenProcessed = SafeProcessEventByHandler(partition, message,
              out string newState, out string newSharedState, out string projectionResult, out EmittedEventEnvelope[] emittedEvents);
            if (hasBeenProcessed)
            {
        ...
        {
            var hasBeenProcessed = SafeProcessPartitionDeletedByHandler(partition, deletedPosition,
                out string newState, out string projectionResult);
            if (hasBeenProcessed)
            {
        ...
        public override void NewCheckpointStarted(CheckpointTag at)
        {
            ...
            {
                if (_logger.IsDebugLevelEnabled()) _logger.LogDebug("Starting a checkpoint in non-runnable state");
                return;
            }
