...
namespace EventStore.Projections.Core.Services.Processing
{
  public class ProjectionCheckpoint : IDisposable, IEmittedStreamContainer, IEventWriter
  {
    ...
    {
#if DESKTOPCLR
      return _emittedStreams.Values.AsQueryExpr().Select(v => v.GetWritePendingEvents()).Sum().Run();
#else
    ...
    {
#if DESKTOPCLR
      return _emittedStreams.Values.AsQueryExpr().Select(v => v.GetWritesInProgress()).Sum().Run();
#else
    ...
    {
#if DESKTOPCLR
      return _emittedStreams.Values.AsQueryExpr().Select(v => v.GetReadsInProgress()).Sum().Run();
#else
