...
namespace EventStore.Core.Index
{
  ...
  public partial class PTable : ISearchTable, IDisposable
  {
                   ...
                   bool skipIndexVerify = false)
    {
      ...
      Ensure.Positive(maxReaders, nameof(maxReaders));
      Ensure.Nonnegative(depth, nameof(depth));
      ...
      if (traceEnabled) Log.LogTrace("Loading "+(skipIndexVerify?"":"and Verification ")+"of PTable '{0}' started...", Path.GetFileName(Filename));
      ...
      {
        Log.LogError(
          "Unable to create midpoints for PTable '{0}' ({1} entries, depth {2} requested). Performance hit will occur. OOM Exception.",
          Path.GetFileName(Filename), Count, depth);
      }
      if (traceEnabled)
      {
        Log.LogTrace("Loading PTable (Version: {0}) '{1}' ({2} entries, cache depth {3}) done in {4}.",
    ...
    internal Midpoint[] CacheMidpointsAndVerifyHash(int depth, bool skipIndexVerify)
    {
      ...
      if (depth < 0 || depth > 30) { throw new ArgumentOutOfRangeException("depth"); }
      var count = Count;
      if (count == 0 || depth == 0) { return null; }

      if (skipIndexVerify) { Log.LogDebug("Disabling Verification of PTable"); }
    ...
    public bool TryGetOneValue(ulong stream, long number, out long position)
    {
      ...
      if (TryGetLargestEntry(hash, number, number, out IndexEntry entry))
      {
    ...
    private bool TryGetLargestEntry(ulong stream, long startNumber, long endNumber, out IndexEntry entry)
    {
      ...
      if (startKey.GreaterThan(_maxEntry) || endKey.SmallerThan(_minEntry)) { return false; }
      ...
      try
      {
        ...
        {
          throw new Exception($"candEntry ({candEntry.Stream}@{candEntry.Version}) > startKey {startKey}, stream {stream}, startNum {startNumber}, endNum {endNumber}, PTable: {Filename}.");
        }

        if (candKey.SmallerThan(startKey)) { return false; }
        entry = candEntry;
        return true;
      }
      finally
    ...
    private bool TryGetSmallestEntry(ulong stream, long startNumber, long endNumber, out IndexEntry entry)
    {
      ...
      try
      {
        ...
        if (candidateKey.GreaterThan(endKey)) { return false; }
        entry = candEntry;
        return true;
      }
      finally
    ...
    public IEnumerable<IndexEntry> GetRange(ulong stream, long startNumber, long endNumber, int? limit = null)
    {
      ...
      if (startKey.GreaterThan(_maxEntry) || endKey.SmallerThan(_minEntry)) { return result; }
      ...
      try
      {
        ...
        for (long i = high, n = Count; i < n; ++i)
        {
          ...
          {
            throw new Exception($"entry ({entry.Stream}@{entry.Version}) > endKey {startKey}, stream {stream}, startNum {startNumber}, endNum {endNumber}, PTable: {Filename}.");
          }

          if (candidateKey.SmallerThan(startKey)) { return result; }
          result.Add(entry);
    ...
    private Range LocateRecordRange(IndexEntryKey key)
    {
      ...
      if (midpoints == null) { return new Range(0, Count - 1); }
    ...
    private void OnAllWorkItemsDisposed()
    {
      ...
      if (_deleteFile) { File.Delete(_filename); }
      _destroyEvent.Set();
    }

    public void WaitForDisposal(int timeout)
    {
      if (!_destroyEvent.Wait(timeout)) { throw new TimeoutException(); }
    }
