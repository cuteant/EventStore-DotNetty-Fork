...
namespace EventStore.Core.Index
{
    public unsafe partial class PTable
    {
        ...
        public static PTable FromMemtable(IMemTable table, string filename, int cacheDepth = 16, bool skipIndexVerify = false)
        {
            ...
            if (Log.IsTraceLevelEnabled()) Log.LogTrace("Dumped MemTable [{0}, {1} entries] in {2}.", table.Id, table.Count, sw.Elapsed);
        ...
        public static PTable MergeTo(IList<PTable> tables, string outputFile, Func<string, ulong, ulong> upgradeHash, Func<IndexEntry, bool> existsAt, Func<IndexEntry, Tuple<string, bool>> readRecord, byte version, int cacheDepth = 16, bool skipIndexVerify = false)
        {
            ...
            if (Log.IsTraceLevelEnabled()) Log.LogTrace("PTables merge started.");
            ...
            if (Log.IsTraceLevelEnabled()) Log.LogTrace("PTables merge finished in {0} ([{1}] entries merged into {2}).",
        ...
        {
            if (Log.IsTraceLevelEnabled()) Log.LogTrace("PTables merge started (specialized for <= 2 tables).");
            var watch = Stopwatch.StartNew();
        ...
        private static void WriteMidpointsTo(BufferedStream bs, FileStream fs, byte version, int indexEntrySize, byte[] buffer, long dumpedEntryCount, long numIndexEntries, long requiredMidpointCount, List<Midpoint> midpoints)
        {
            ...
            if(dumpedEntryCount == numIndexEntries && requiredMidpointCount == midpoints.Count){ //if these values don't match, something is wrong
                ...
                if(Log.IsDebugLevelEnabled()) Log.LogDebug("Cached {0} index midpoints to PTable", midpointsWritten);
            }
            else
                if (Log.IsDebugLevelEnabled()) Log.LogDebug("Not caching index midpoints to PTable due to count mismatch. Table entries: {0} / Dumped entries: {1}, Required midpoint count: {2} /  Actual midpoint count: {3}", numIndexEntries, dumpedEntryCount, requiredMidpointCount, midpoints.Count);
