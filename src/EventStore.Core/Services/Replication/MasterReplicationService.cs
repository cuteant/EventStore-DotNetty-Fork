...
namespace EventStore.Core.Services.Replication
{
                                            ...
                                            IHandle<ReplicationMessage.GetReplicationStats>
  {
    ...
    public void Handle(SystemMessage.StateChangeMessage message)
    {
      ...
      if (message.State == VNodeState.ShuttingDown) { _stop = true; }
    }
    ...
    {
      if (_subscriptions.TryGetValue(message.SubscriptionId, out ReplicaSubscription subscription))
      {
    ...
    private bool ManageSubscriptions()
    {
      ...
      foreach (var subscription in _subscriptions.Values)
      {
        ...
        {
          _subscriptions.TryRemove(subscription.SubscriptionId, out ReplicaSubscription tmp);
          subscription.Dispose();
          continue;
        }
    ...
    private void ManageRoleAssignments(IEnumerable<ReplicaSubscription> subscribers)
    {
      ...
      for (int k = 0; k < laggedSlaves; ++k)
      {
        ...
        if (cloneIndex > slaveIndex) { break; }
