...
namespace EventStore.Core.Services.PersistentSubscription
{
      ...
      IHandle<MonitoringMessage.GetStreamPersistentSubscriptionStats>
  {
    ...
    public void Handle(SystemMessage.StateChangeMessage message)
    {
      ...
      if (Log.IsDebugLevelEnabled()) Log.LogDebug($"Subscriptions received state change to {_state} stopping listening.");
    ...
    {
      if (Log.IsDebugLevelEnabled()) Log.LogDebug("Subscriptions Became Master so now handling subscriptions");
      InitToEmpty();
      _handleTick = true;
    ...
    public void Handle(ClientMessage.CreatePersistentSubscription message)
    {
      ...
      if (Log.IsDebugLevelEnabled()) Log.LogDebug($"create subscription {key}");
    ...
    public void Handle(ClientMessage.UpdatePersistentSubscription message)
    {
      if (!_started) { return; }
      var key = BuildSubscriptionGroupKey(message.EventStreamId, message.GroupName);
      if (Log.IsDebugLevelEnabled()) Log.LogDebug($"update subscription {key}");
      var streamAccess = _readIndex.CheckStreamAccess(SystemStreams.SettingsStream, StreamAccessType.Write, message.User);
                                            ...
                                            TimeSpan messageTimeout)
    {
      ...
      if (!_subscriptionTopics.TryGetValue(eventStreamId, out List<PersistentSubscription> subscribers))
      {
    ...
    {
      if (!_started) { return; }
      if (Log.IsDebugLevelEnabled()) Log.LogDebug($"delete subscription {message.GroupName}");
      var streamAccess = _readIndex.CheckStreamAccess(SystemStreams.SettingsStream, StreamAccessType.Write, message.User);
      ...
      var key = BuildSubscriptionGroupKey(message.EventStreamId, message.GroupName);
      if (!_subscriptionsById.TryGetValue(key, out PersistentSubscription subscription))
      {
        message.Envelope.ReplyWith(new ClientMessage.DeletePersistentSubscriptionCompleted(message.CorrelationId,
    ...
    public void Handle(TcpMessage.ConnectionClosed message)
    {
      ...
      if (Log.IsDebugLevelEnabled()) Log.LogDebug($"Lost connection from {message.Connection.RemoteEndPoint}");
    ...
    {
      if (!_started) { return; }
      var streamAccess = _readIndex.CheckStreamAccess(message.EventStreamId, StreamAccessType.Read, message.User);
      ...
      if (!_subscriptionTopics.TryGetValue(message.EventStreamId, out List<PersistentSubscription> subscribers))
      {
      ...
      if (Log.IsDebugLevelEnabled()) Log.LogDebug("New connection to persistent subscription {0}.", message.SubscriptionId);
    ...
    {
      if (!_subscriptionTopics.TryGetValue(eventStreamId, out List<PersistentSubscription> subscriptions)) { return; }
    ...
    private ResolvedEvent ResolveLinkToEvent(EventRecord eventRecord, long commitPosition)
    {
      if (eventRecord.EventType == SystemEventTypes.LinkTo)
      {
        try
        {
          string[] parts = Helper.UTF8NoBom.GetStringWithBuffer(eventRecord.Data).Split('@');
    ...
    {
      if (!_started) return;
      if (_subscriptionsById.TryGetValue(message.SubscriptionId, out PersistentSubscription subscription))
      {
    ...
    {
      if (Log.IsDebugLevelEnabled()) Log.LogDebug("Saving Configuration.");
    ...
    private void HandleSaveConfigurationCompleted(Action continueWith, ClientMessage.WriteEventsCompleted obj)
    {
      switch (obj.Result)
      {
        ...
        case OperationResult.PrepareTimeout:
          if (Log.IsInformationLevelEnabled()) Log.LogInformation("Timeout while trying to save subscription configuration.");
