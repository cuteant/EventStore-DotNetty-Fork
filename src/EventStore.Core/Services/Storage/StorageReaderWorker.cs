...
namespace EventStore.Core.Services.Storage
{
        ...
        IHandle<StorageMessage.CheckStreamAccess>
    {
        ...
        void IHandle<ClientMessage.ReadEvent>.Handle(ClientMessage.ReadEvent msg)
        {
            ...
            {
                if (Log.IsDebugLevelEnabled()) Log.LogDebug("Read Event operation has expired for Stream: {0}, Event Number: {1}. Operation Expired at {2}", msg.EventStreamId, msg.EventNumber, msg.Expires);
                return;
            }
        ...
        void IHandle<ClientMessage.ReadStreamEventsForward>.Handle(ClientMessage.ReadStreamEventsForward msg)
        {
            if (msg.Expires < DateTime.UtcNow)
            {
                if (Log.IsDebugLevelEnabled()) Log.LogDebug("Read Stream Events Forward operation has expired for Stream: {0}, From Event Number: {1}, Max Count: {2}. Operation Expired at {3}", msg.EventStreamId, msg.FromEventNumber, msg.MaxCount, msg.Expires);
                return;
            }
        ...
        void IHandle<ClientMessage.ReadStreamEventsBackward>.Handle(ClientMessage.ReadStreamEventsBackward msg)
        {
            if (msg.Expires < DateTime.UtcNow)
            {
                if (Log.IsDebugLevelEnabled()) Log.LogDebug("Read Stream Events Backward operation has expired for Stream: {0}, From Event Number: {1}, Max Count: {2}. Operation Expired at {3}", msg.EventStreamId, msg.FromEventNumber, msg.MaxCount, msg.Expires);
                return;
            }
        ...
        void IHandle<ClientMessage.ReadAllEventsForward>.Handle(ClientMessage.ReadAllEventsForward msg)
        {
            if (msg.Expires < DateTime.UtcNow)
            {
                if (Log.IsDebugLevelEnabled()) Log.LogDebug("Read All Stream Events Forward operation has expired for C:{0}/P:{1}. Operation Expired at {2}", msg.CommitPosition, msg.PreparePosition, msg.Expires);
                return;
            }
        ...
        void IHandle<ClientMessage.ReadAllEventsBackward>.Handle(ClientMessage.ReadAllEventsBackward msg)
        {
            if (msg.Expires < DateTime.UtcNow)
            {
                if (Log.IsDebugLevelEnabled()) Log.LogDebug("Read All Stream Events Backward operation has expired for C:{0}/P:{1}. Operation Expired at {2}", msg.CommitPosition, msg.PreparePosition, msg.Expires);
                return;
            }
        ...
        void IHandle<StorageMessage.CheckStreamAccess>.Handle(StorageMessage.CheckStreamAccess msg)
        {
            if (msg.Expires < DateTime.UtcNow)
            {
                if (Log.IsDebugLevelEnabled()) Log.LogDebug("Check Stream Access operation has expired for Stream: {0}. Operation Expired at {1}", msg.EventStreamId, msg.Expires);
                return;
            }
