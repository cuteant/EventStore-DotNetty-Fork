        ...
        IHandle<UserManagementMessage.UserManagementServiceInitialized>
    {
        ...
        private void WriteScavengeIndexInitializedEvent()
        {
            ...
            _ioDispatcher.WriteEvent(metaStreamId, ExpectedVersion.Any, metaStreamEvent, SystemAccount.Principal, m =>
            {
                ...
                {
                    Log.LogError("Failed to write the $maxAge of {0} days metadata for the {1} stream. Reason: {2}", _scavengeHistoryMaxAge, SystemStreams.ScavengesStream, m.Result);
                }
            });
            ...
            _ioDispatcher.WriteEvent(SystemStreams.ScavengesStream, ExpectedVersion.NoStream, indexInitializedEvent, SystemAccount.Principal, m =>
            {
                ...
                {
                    Log.LogError("Failed to write the {0} event to the {1} stream. Reason: {2}", SystemEventTypes.ScavengeIndexInitialized, SystemStreams.ScavengesStream, m.Result);
                }
            });
        }
        ...
        private void WriteScavengeStartedEvent(string streamName, Guid scavengeId)
        {
            ...
            _ioDispatcher.WriteEvent(metaStreamId, ExpectedVersion.Any, metaStreamEvent, SystemAccount.Principal, m =>
            {
                if (m.Result != OperationResult.Success)
                {
                    Log.LogError("Failed to write the $maxAge of {0} days metadata for the {1} stream. Reason: {2}", _scavengeHistoryMaxAge, streamName, m.Result);
                }
            });
        ...
        private void WriteScavengeIndexEventCompleted(ClientMessage.WriteEventsCompleted msg, Event linkToEvent, int retryCount)
        {
            if (msg.Result != OperationResult.Success)
            {
                if (retryCount > 0)
                {
                    Log.LogError("Failed to write an event to the {0} stream. Retrying {1}/{2}. Reason: {3}", SystemStreams.ScavengesStream, (MaxRetryCount - retryCount) + 1, MaxRetryCount, msg.Result);
                    WriteScavengeIndexEvent(linkToEvent, --retryCount);
                }
                else
                {
                    Log.LogError("Failed to write an event to the {0} stream. Retry limit of {1} reached. Reason: {2}", SystemStreams.ScavengesStream, MaxRetryCount, msg.Result);
                }
            }
        }
