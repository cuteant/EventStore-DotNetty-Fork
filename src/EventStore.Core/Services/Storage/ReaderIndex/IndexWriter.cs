    ...
    public class IndexWriter : IIndexWriter
    {
        ...
        public CommitCheckResult CheckCommit(string streamId, long expectedVersion, IEnumerable<Guid> eventIds)
        {
            ...
            if (expectedVersion == ExpectedVersion.Any || expectedVersion == ExpectedVersion.StreamExists)
            {
                ...
                {
                    if (!_committedEvents.TryGetRecord(eventId, out EventInfo prepInfo) || prepInfo.StreamId != streamId)
                    {
                    ...
                    if (first) { startEventNumber = prepInfo.EventNumber; }
            ...
            if (expectedVersion < curVersion)
            {
                ...
                foreach (var eventId in eventIds)
                {
                    ...
                    if (_committedEvents.TryGetRecord(eventId, out EventInfo prepInfo)
        ...
        private bool GetTransactionInfoUncached(long writerCheckpoint, long transactionId, out TransactionInfo transactionInfo)
        {
            using (var reader = _indexBackend.BorrowReader())
            {
                ...
                {
                    if (result.LogRecord.LogPosition < transactionId) { break; }
        ...
        {
            if (_streamVersions.TryGet(streamId, out long lastEventNumber))
            {
