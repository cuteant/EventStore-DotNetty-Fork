...
namespace EventStore.Core.Services
{
        ...
        IHandle<StorageMessage.EventCommitted>
    {
        ...
        {
            if (!_subscriptionTopics.TryGetValue(eventStreamId, out List<Subscription> subscribers))
            {
        ...
        {
            if (_subscriptionsById.TryGetValue(correlationId, out Subscription subscription))
                DropSubscription(subscription, sendDropNotification: true);
        }
        ...
        {
            if (!_pollTopics.TryGetValue(streamId, out List<PollSubscription> pollTopic))
            {
        ...
        {
            if (!_subscriptionTopics.TryGetValue(eventStreamId, out List<Subscription> subscriptions))
            {
            ...
            for (int i = 0, n = subscriptions.Count; i < n; i++)
            {
                ...
                if (commitPosition <= subscr.LastCommitPosition || evnt.EventNumber <= subscr.LastEventNumber) { continue; }
