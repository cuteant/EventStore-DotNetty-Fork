...
{
  /// <summary>Fluent builder used to configure <see cref="ClusterSettings" /> for connecting to a cluster
  /// using gossip seeds.</summary>
  public class GossipSeedClusterSettingsBuilder
  {
    ...
    public GossipSeedClusterSettingsBuilder SetMaxDiscoverAttempts(int maxDiscoverAttempts)
    {
      ...
      {
        throw new ArgumentOutOfRangeException(nameof(maxDiscoverAttempts), $"{nameof(maxDiscoverAttempts)} value is out of range: {maxDiscoverAttempts}. Allowed range: [1, infinity].");
      }
    ...
    /// <summary>Whether or nto to randomly choose a node that's alive from the known nodes.</summary>
    /// <returns>A <see cref="GossipSeedClusterSettingsBuilder"/> for further configuration.</returns>
